...
from grus.render import render_set_svg, render_svg, render_svgs
...
def test_empty_set_is_a_minimal_valid_svg() -> None:
    svg = render_set_svg(pb.PedigreeSet())
...
def test_set_stacks_each_pedigree_as_a_nested_svg_with_its_title() -> None:
    svg = render_set_svg(pb.PedigreeSet(pedigrees=[_trio("Family 1"), _trio("Family 2")]))
...
def test_tile_body_preserves_the_single_render_content() -> None:
    ped = _trio("Family 1")
    single = render_svg(ped)
    figure = render_set_svg(pb.PedigreeSet(pedigrees=[ped]))
...
def test_deferred_pedigree_becomes_a_placeholder_and_others_still_render() -> None:
    svg = render_set_svg(pb.PedigreeSet(pedigrees=[_trio("ok"), _two_parented()]))
...
def test_render_svgs_returns_one_document_per_pedigree() -> None:
    svgs = render_svgs(pb.PedigreeSet(pedigrees=[_trio("Family 1"), _trio("Family 2")]))
...
def test_render_svgs_defers_per_pedigree_without_dropping_others() -> None:
    svgs = render_svgs(pb.PedigreeSet(pedigrees=[_trio("ok"), _two_parented()]))
    assert len(svgs) == 2
...
def test_render_svgs_empty_set_is_empty_list() -> None:
    assert render_svgs(pb.PedigreeSet()) == []


def test_render_set_is_deterministic() -> None:
    s = pb.PedigreeSet(pedigrees=[_trio("A"), _trio("B")])
    assert render_set_svg(s) == render_set_svg(s)


def test_title_is_xml_escaped() -> None:
    svg = render_set_svg(pb.PedigreeSet(pedigrees=[_trio("A & B <x>")]))
    assert "A &amp; B &lt;x&gt;" in svg
